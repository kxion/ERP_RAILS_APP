  ...
  module Ebay
    module Api
      module Base
        ...
        def call_json(url, body, options = {})
          options['marketplace_id'] ||= 'EBAY-US'
          uri = URI.parse('https://api.ebay.com')
          uri = URI.parse('https://api.sandbox.ebay.com') if self.sandbox?
          http = Net::HTTP.new(uri.host, uri.port)
          http.use_ssl = true
          http.verify_mode = OpenSSL::SSL::VERIFY_NONE
          request = Net::HTTP::Post.new(url)
          request.add_field('Authorization', "TOKEN #{self.auth_token}")
          request.add_field('Content-Type', 'application/json')
          request.add_field('X-EBAY-C-MARKETPLACE-ID', options['marketplace_id'])
