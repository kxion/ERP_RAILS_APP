    ...
    module Api
      ...
      <RequesterCredentials>
        ...
        def send
          ...
          File.open('ebay_response.txt', 'wb') { |file| file.write(response.body) }  if debug
        ...
        def self.xml(*args)
          args.map do |structure|
            case structure
              when Hash then
                structure.map do |k, value|
                  if value.is_a?(Array) && value.select { |v| v.is_a?(String) }.size == value.size
                    value.map { |v| "<#{k.to_s}>#{HTMLEntities.new.encode(v)}</#{k.to_s}>" }.join
                  else
                    "<#{k.to_s}>#{xml(value)}</#{k.to_s}>"
                  end
                end.join
              when Array then
                structure.map { |v| xml(v) }.join
              else
                HTMLEntities.new.encode(self.serialize_input(structure).to_s)
            end
          end.join
        end
